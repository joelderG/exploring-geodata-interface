// Detects rightward drag gestures on the secondary touch point (context menu).
...
export class ContextMenuDragRightRecognizer implements GestureRecognizer {
  name = 'context-menu-drag-right';
  private cooldownUntil = 0;
  private activeTouchId: number | null = null;

  // Thresholds for a valid rightward drag (normalized coords).
  ...
  update(frame: TouchFrame, state: GestureState): GestureEvent[] {
    ...
    if (dx < this.minHorizontalDelta) return []; // not rightward
